...
const TH = new Uint8Array(40);   // 8 (HD) + 32 (publicKey) = 40
...
function fastBase64(fl) {
    ...
    return a;   // без обрезки — адрес всегда 48 символов
}
...
// ─── Батчевый цикл через setTimeout — гарантирует доставку postMessage ───────
const BATCH_SIZE = 300;   // итераций за один батч
...
function tick() {
    ...
    try {
        for (let j = 0; j < BATCH_SIZE; j++) {
            ...
            const hs = sha256sync(TH);
            PL[0] = 0x51; PL[1] = 0x00; PL.set(hs, 2);
            const cr = crc16(PL);
            FL.set(PL, 0); FL[34] = cr>>8; FL[35] = cr&0xFF;
            const addr = fastBase64(FL);
            const r = checkStart(addr);
            localChecked++;
            if (r) {
        ...
        postMessage({type:'error', msg:'Batch error: '+e.message});
    }
    if (localChecked > 0) postMessage({type:'progress', checked:localChecked});
    setTimeout(tick, 0);   // уступаем event loop, потом сразу продолжаем
}
